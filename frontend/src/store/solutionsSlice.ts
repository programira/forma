...
const solutionsSlice = createSlice({
  ...
  reducers: {
    ...
    toggleSolution(state, action: PayloadAction<{ id: string }>) {
      ...
      const isSelected = state.selectedSolutions.some((solution) => solution.id === solutionId);
      ...
      if (isSelected) {
        ...
        const solution = state.solutions.find((solution) => solution.id === solutionId);
        if (solution) {
    ...
    togglePolygonSelection(state, action: PayloadAction<string>) {
      ...
      if (isSelected) {
        state.selectedPolygons = state.selectedPolygons.filter((id) => id !== polygonId); // Deselect
      } else {
    ...
    performUnion(state) {
      ...
      const { unionResult, totalArea } = performUnionUsingFeatureCollection(selectedPolygonFeatures);
      ...
      // const unionSolution: WritableDraft<FeatureCollection> = {
      ...
      //     unionResult, // Add the union result
      //   ],
      // };
 ...
 const newUnionFeature = {
  ...
  // Replace the selected polygons in the solutions
  state.solutions = state.solutions.map((solution) => {
    // Check if the solution contains any of the selected polygons
    ...
    if (containsSelectedPolygons) {
      ...
      }as WritableDraft<FeatureCollection>; // Explicit cast for the entire solution
    }

    // Keep other solutions unchanged
    return solution;
  });

  console.log('state.solutions', state.solutions);

  // Update the union result and total area
  // state.unionResult = newUnionFeature;
  state.totalArea = totalArea;
  ...
  state.selectedPolygons = [];
    ...
    performIntersect(state) {
      ...
      const { intersectResult, totalArea } = performIntersectUsingFeatureCollection(selectedPolygonFeatures);
